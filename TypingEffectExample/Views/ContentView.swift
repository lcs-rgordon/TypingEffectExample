...
struct TypeOnViewModifier: ViewModifier {
    ...
    init(message: String, debug: Bool = false) {
...
extension View {
    func typeOn(message: String, debugLayout: Bool = false) -> some View {
        self.modifier(TypeOnViewModifier(message: message, debug: debugLayout))
    }
}
